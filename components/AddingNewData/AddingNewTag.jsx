...
const categoryID=[]
  ...
  axios.put("http://localhost:3000/api/individualtags", {tagSubmission}).then(response => {
    console.log(response)
   setIsPending(false);
}).catch(error => {
console.log("this is error", error);
  ...
  <form onSubmit={handleNameSubmission}>
    ...
    <Select 
     options={categoryList.map((opt,index) => ({ label: opt.name, value: opt.individualname}))}
      isMulti
      isSearchable
      placeholder="Add a category/categories"
      onChange={(opt) => setCategory(opt.map(category=>category.label))}
